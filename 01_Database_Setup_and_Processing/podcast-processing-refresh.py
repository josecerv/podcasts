#!/usr/bin/env python3
"""
Incremental Podcast RSS Feed Processor
A script to process podcast RSS feeds, collecting only new episodes published since the latest
episode in the database for each podcast. Maintains consistent episode IDs and only adds
new content without modifying existing data.
"""
...
class PodcastProcessor:
    ...
    def process_feed_incremental(self, rss_url: str, podcast_id: str, latest_date) -> List[dict]:
        """
        Process a single podcast feed with timeout, but only return episodes
        published after the latest_date for this podcast.
        """
        try:
            original_timeout = socket.getdefaulttimeout()
            socket.setdefaulttimeout(TIMEOUT)
            ...
            local_episodes = []
            for entry in feed.entries:
                episode_data = self.process_entry_incremental(entry, podcast_id, latest_date)
                if episode_data:
                    local_episodes.append(episode_data)
    ...
    def process_entry_incremental(self, entry: dict, podcast_id: str, latest_date) -> Optional[dict]:
        """Process a single feed entry, but only if it's newer than latest_date."""
        try:
            published_parsed = entry.get('published_parsed')
            published_date = datetime(*published_parsed[:6]) if published_parsed else None
            
            # Skip if no publication date or not in 2024/2025
            if not published_date or published_date.year not in (2024, 2025):
                return None
                
            # Skip if this episode is not newer than our latest date
            if latest_date and published_date <= latest_date:
                return None
                
            # If we're here, this is a new episode - continue with normal processing
            raw_description = get_robust_description(entry)
            clean_description = clean_text(raw_description)
            if not clean_description:
                return None
...
def process_single_podcast_incremental(row_dict, processor: PodcastProcessor) -> List[dict]:
    """
    Process a single podcast feed, but only look for episodes newer than the latest_date.
    """
    try:
        rss_url = row_dict.get('rss')
        podcast_id = row_dict.get('podcast_id')
        latest_date = row_dict.get('latest_date')
        
        if not rss_url:
            return []
            
        episodes = processor.process_feed_incremental(rss_url, podcast_id, latest_date)
        
        with processed_count.get_lock():
            processed_count.value += 1
            current_count = processed_count.value
            if len(episodes) > 0:  # Changed to any new episodes
                with successful_podcasts.get_lock(), batch_successful.get_lock():
                    successful_podcasts.value += 1
                    batch_successful.value += 1
            if current_count % BATCH_SIZE == 0:
                with batch_successful.get_lock():
                    batch_rate = (batch_successful.value / BATCH_SIZE) * 100
                    print(f"\n{current_count:,} podcasts processed:")
                    print(f" - Current batch with new episodes: {batch_successful.value:,} ({batch_rate:.1f}% of {BATCH_SIZE})")
                    print(f" - Total podcasts with new episodes so far: {successful_podcasts.value:,}")
                    batch_successful.value = 0
...
def write_compressed_sql(episodes: List[dict], base_filename: str = 'podcast_episodes', load_to_postgres: bool = False):
    ...
    with open(sql_filename, 'w', encoding='utf-8') as f:
        ...
        for i, episode in enumerate(episodes, 1):
            ...
            insert_statement = (
                f"INSERT INTO episodes ({', '.join(columns)}) "
...
def write_compressed_sql_incremental(episodes: List[dict], base_filename: str = 'podcast_episodes_incremental', load_to_postgres: bool = False):
    """
    Write only the new episodes to a timestamped .sql file with only INSERT statements.
    """
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    sql_filename = f'{base_filename}_{timestamp}.sql'
    compressed_filename = f'{sql_filename}.gz'

    print(f"\nWriting incremental SQL file: {sql_filename}")
    with open(sql_filename, 'w', encoding='utf-8') as f:
        # Only INSERT statements, no DROP/CREATE TABLE
        total_episodes = len(episodes)
        for i, episode in enumerate(episodes, 1):
            if i % 10000 == 0:
                print(f"Writing episode {i:,} of {total_episodes:,}")
...
def process_podcast_incremental_wrapper(args):
    """
    Wrapper function to unpack arguments for process_single_podcast_incremental.
    """
    row_dict, processor = args
    return process_single_podcast_incremental(row_dict, processor)
...
def process_podcasts_incremental(csv_path: str = 'podcasts_final_sample.csv', load_to_postgres: bool = False):
    """
    Incremental processing function that only adds new episodes published 
    after the most recent episode date in the database for each podcast.
    """
    start_time = time.time()
    try:
        # First, connect to the database and get latest episode date for each podcast
        import psycopg2
        ...
        print("Fetching latest episode dates from database...")
        cursor.execute("""
            SELECT podcast_id, MAX(episode_published) 
            FROM episodes 
            GROUP BY podcast_id
        """)
        ...
        cursor.close()
        conn.close()
        print(f"Found latest dates for {len(latest_dates):,} podcasts in database")
        ...
        podcast_data = [
            {
                'rss': row.rss if pd.notna(row.rss) else None,
                'podcast_id': row.podcast_id if pd.notna(row.podcast_id) else None,
                'latest_date': latest_dates.get(row.podcast_id if pd.notna(row.podcast_id) else None)
            }
            for row in podcasts_df.itertuples()
        ]
        ...
        print("Starting podcast processing...")
        with concurrent.futures.ProcessPoolExecutor(max_workers=num_workers) as executor:
            for batch_results in executor.map(process_podcast_incremental_wrapper, process_args):
                if batch_results:
                    all_episodes.extend(batch_results)
        
        print(f"\nTotal new episodes collected: {len(all_episodes):,}")
        
        # Verify unique episodes and write to SQL
        if all_episodes:
            unique_episodes = verify_unique_episodes(all_episodes)
            print(f"Total unique new episodes: {len(unique_episodes):,}")
            write_compressed_sql_incremental(unique_episodes, load_to_postgres=load_to_postgres)
        else:
        ...
        logger.error(f"Critical error during incremental processing: {e}")
        raise
...
def main():
    ...
    """
    try:
        print(f"Starting incremental podcast RSS feed processing at: {datetime.now()}")
        def signal_handler(signum, frame):
            print("\nReceived interrupt signal. Finishing current batch before exiting...")
            raise KeyboardInterrupt
        signal.signal(signal.SIGINT, signal_handler)
        
        # Use incremental processing instead of full processing
        process_podcasts_incremental(csv_path='podcasts_final_sample.csv', load_to_postgres=True)
